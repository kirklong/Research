...
function createBootStrapMatrix(powList,nTrials,iterNum)  #nTrials for random points, iterNum for number of times this function will run
    ...
    while counter<nTrials
        if counter%10==0
            print("Iteration ",iterNum," ",format(counter/nTrials*100,precision=2),"% complete \r") #output % tracker every 10
        end
...
function genStdMatrix(iterNum,powList,nTrials)
    allMatrices=[]
    for i=1:iterNum
        push!(allMatrices,createBootStrapMatrix(powList,nTrials,i))
    ...
    @showprogress 1 "Computing part 2..." for f=2:(nk+1) #current frequency, +1 shift because 1st slice is not freq dependent
        ...
        for i=1:101 #row counter
            for j=1:101 #col counter
                ...
                while stop==false
                    currentMatrix=allMatrices[Int(m)]
                    currentEntry=currentMatrix[i,j,Int(f)]
                    data[m]+=currentEntry
                    if m<length(allMatrices)
                        m+=1
                    else #reached the end of loop, find std and place in appropriate cell
                        currentStd=std(data)
                        stdMatrix[i,j,Int(f)]=currentStd
                        stop=true
                    end
                end
            end
