#performs FFT on photon files selected by regular expression

using DelimitedFiles, FFTW, Plots, CSV, Statistics, DataFrames

#################################################################################
function freqarr(delt,nfreqs)

    fspace = 1.0/(nfreqs*delt)
    freqs=[n*fspace for n in 1:nfreqs]
...
#################################################################################
function ffttimes(times,delt)

    tbins=bintimes(times,delt)
    fftx = fft(tbins)
    return fftx

end
#################################################################################
function fftout(fft,freqs)
...
#################################################################################
function fftfancy(times,delt,save=0,plot=0,bigout=0,fhead="",tdir=".",inst="none")
    ...
    #big = [x->pow1881(x)/mean1881 .> 10]
    #bigpow=pow[pow .> 10]

    #plot(powx/meanp)

    if occursin("current",tdir)
    end

    if occursin("cxo",inst)
      print(inst)
    elseif occursin("roshri",inst)
      print(inst)

    elseif occursin("rospspc",inst)
      print(inst)

      elseif occursin("xmm",inst)
        print(inst)
    end
...
function fftdir(datadir::String,suff::String,delt::Float64)
    ...
    #println(filehd,"\n")
        ...
        plot(myfreqs[2:end],mypow[2:end]/avepow,title=titlestr,xlabel="freq",ylabel="Power")
...
##############################################################################
#Takes a list of photon arrival times and creates binned occupancy array for FFT
#DJM, Jan 2019
function bintimes(ptimes,dt,setzero=1,tdur=0)
    #tdatin="/data/heasarc/heasarc_cxoxassist.tdat"
    #tdat = readlines(tdatin)

    tmin = minimum(ptimes)
    if setzero == 1
      ptimes = ptimes .- tmin     #set t=0 at first time
    end

#if no limit to time length, calculalte using actual data
    if tdur == 0
       tmin=minimum(ptimes)
       tmax = maximum(ptimes)
       tdur = tmax-tmin
...
#setup zero bins with dt widths
    nbins = ceil(Int,tdur/dt)
    cts = zeros(nbins)
    tbins = floor.(Int,ptimes/dt) .+1
